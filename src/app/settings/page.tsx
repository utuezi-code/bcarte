          ...
          {tab === 1 && (
            <div className="card space-y-4">
              ...
              <form onSubmit={handlePasswordChange} className="space-y-4">
                ...
                {pwSuccess && (
                  <p className="text-sm text-green-600 bg-green-50 border border-green-100 rounded-lg px-3 py-2 flex items-center gap-2">
                    <IconCheck size={14} /> Mot de passe modifié avec succès
                  </p>
                )}

                <button type="submit" disabled={pwLoading} className="btn-primary flex items-center gap-2 px-5 py-2.5 disabled:opacity-60">
                  {pwLoading ? <IconLoader2 size={16} className="animate-spin" /> : pwSuccess ? <IconCheck size={16} /> : null}
                  {pwLoading ? 'Modification…' : pwSuccess ? 'Modifié !' : 'Modifier le mot de passe'}
                </button>
              </form>
            </div>
          )}
              ...
              {[
                ...
                <div key={n.key} className="flex items-center justify-between py-2 border-b border-[#F3F4F6] last:border-0">
                  ...
                  <button
                    ...
                    className={`rounded-full transition-colors relative flex-shrink-0 ${notifs[n.key as keyof typeof notifs] ? 'bg-primary' : 'bg-[#E5E7EB]'}`}
                    style={{ width: 40, height: 22 }}
                  >
                    <span className={`absolute top-0.5 rounded-full bg-white shadow transition-all ${notifs[n.key as keyof typeof notifs] ? 'right-0.5' : 'left-0.5'}`}
          ...
          {tab === 3 && (
            <div className="card space-y-4">
              ...
              <div className="flex items-center justify-between py-2">
                ...
                <div className="rounded-full bg-primary relative flex-shrink-0" style={{ width: 40, height: 22 }}>
                  <span className="absolute right-0.5 top-0.5 rounded-full bg-white shadow" style={{ width: 18, height: 18 }} />
                </div>
              </div>
